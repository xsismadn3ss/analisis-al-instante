...
export function LineChartComponent({ data, xAxisLabel, yAxisLabel }: LineChartProps) {
          ...
          <LineChart data={paginatedData}>
            ...
            {yAxisKeys.map((key ) => (
              <Line
                ...
                strokeWidth={2}
                dot={{ fill: generateColorFromString(key), strokeWidth: 2, r: 4 }}
                activeDot={{ r: 6, stroke: generateColorFromString(key), strokeWidth: 2 }}
              />
            ))}
          </LineChart>
        </ResponsiveContainer>
